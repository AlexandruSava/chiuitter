...
class HomeActivity : AppCompatActivity() {
    ...
    private fun extractText(data: Intent?) {
        data?.let {
            ...
            val textReceived : String? = data.getStringExtra(ComposeActivity.EXTRA_TEXT)

            // Check if text is not null or empty, then set the new "chiuit" content.
            if (!textReceived.isNullOrEmpty()) {
                // Instantiate a new chiuit object that add it to the adapter
                listAdapter.addItem(Chiuit(textReceived))
            }
        }
    }
